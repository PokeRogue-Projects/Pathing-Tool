...
class AttackTypeBoosterModifierTypeGenerator extends ModifierTypeGenerator {
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      const randInt = randSeedInt(totalWeight, undefined, doModifierLogging ? "Generating a move type booster" : "%HIDE");
      let weight = 0;

      const fullweights: integer[] = [];
...
class BaseStatBoosterModifierTypeGenerator extends ModifierTypeGenerator {
  ...
  constructor() {
    super((_party: Pokemon[], pregenArgs?: any[]) => {
      ...
      const randStat: PermanentStat = randSeedInt(Stat.SPD + 1, undefined, doModifierLogging ? "Randomly generating a Vitamin" : "%HIDE");
...
class TempStatStageBoosterModifierTypeGenerator extends ModifierTypeGenerator {
  ...
  constructor() {
    super((_party: Pokemon[], pregenArgs?: any[]) => {
      ...
      const randStat: TempBattleStat = randSeedInt(Stat.ACC, Stat.ATK, doModifierLogging ? "Randomly choosing an X item" : "%HIDE");
...
class SpeciesStatBoosterModifierTypeGenerator extends ModifierTypeGenerator {
  ...
  constructor() {
    super((party: Pokemon[], pregenArgs?: any[]) => {
      ...
      if (totalWeight !== 0) {
        const randInt = randSeedInt(totalWeight, 1, doModifierLogging ? "Randomly choosing a species booster" : "HIDE");
        let weight = 0;
...
export const modifierTypes = {
  ...
  MINT: () => new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
    ...
    return new PokemonNatureChangeModifierType(randSeedInt(getEnumValues(Nature).length, undefined, "Choosing a Mint") as Nature);
  }),
  ...
  TERA_SHARD: () => new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
    ...
    let type: Type;
    if (!randSeedInt(3, undefined, "Choosing whether to give a type from your party")) {
      const partyMemberTypes = party.map(p => p.getTypes(false, false, true)).flat();
      type = randSeedItem(partyMemberTypes, "Choosing a Tera Shard type to give");
    } else {
      type = randSeedInt(64, undefined, "Choosing whether to give a Stellar Shard") ? randSeedInt(18, undefined, "Choosing a type") as Type : Type.STELLAR;
    }
  ...
  BERRY: () => new ModifierTypeGenerator((party: Pokemon[], pregenArgs?: any[]) => {
    ...
    const rand = randSeedInt(12, undefined, "Choosing a Berry");
    if (rand < 2) {
      ...
      randBerryType = berryTypes[randSeedInt(berryTypes.length - 3, undefined, "Choosing a random berry type") + 2];
    }
...
export function getEnemyBuffModifierForWave(tier: ModifierTier, enemyModifiers: PersistentModifier[], scene: BattleScene): EnemyPersistentModifier {
  ...
  let matchingModifier: PersistentModifier;
...
export function getEnemyModifierTypesForWave(waveIndex: integer, count: integer, party: EnemyPokemon[], poolType: ModifierPoolType.WILD | ModifierPoolType.TRAINER, upgradeChance: integer = 0, scene?: BattleScene): PokemonHeldItemModifierType[] {
  const ret = new Array(count).fill(0).map(() => getNewModifierTypeOption(party, poolType, undefined, upgradeChance && !randSeedInt(upgradeChance, undefined, "Chance to upgrade an opponent's item") ? 1 : 0)?.type as PokemonHeldItemModifierType);
  if (!(waveIndex % 1000)) {
...
export function getDailyRunStarterModifiers(party: PlayerPokemon[], scene?: BattleScene): PokemonHeldItemModifier[] {
...
function getNewModifierTypeOption(party: Pokemon[], poolType: ModifierPoolType, tier?: ModifierTier, upgradeCount?: integer, retryCount: integer = 0, allowLuckUpgrades: boolean = true): ModifierTypeOption | null {
  ...
  if (tier === undefined) {
    ...
    if (player && tierValue && allowLuckUpgrades) {
      ...
      let upgraded = false;
      do {
        upgraded = randSeedInt(upgradeOdds, undefined, "Upgrade chance") < 4;
        if (upgraded) {
          upgradeCount++;
        }
      } while (upgraded);
    ...
    if (tier < ModifierTier.MASTER && allowLuckUpgrades) {
      ...
      while (modifierPool.hasOwnProperty(tier + upgradeCount + 1) && modifierPool[tier + upgradeCount + 1].length) {
        if (!randSeedInt(upgradeOdds, undefined, "Upgrade chance 2")) {
          upgradeCount++;
        } else {
          break;
        }
